...
The `$origionalTransactionId` is the transaction ID provided in the original
`$gateway->purchase([...])` request, and is mandatory.
...
Here `$complete_purchase_request` will contain all the data needed to parse
the result.

```php
// Confirms the message signature is valid.
